...
### Fixed
- **Status bar wraps to a second line**: After the v1.4.0 clock was prepended to the footer label, some pages (especially in flow mode and late-book paged pages with larger chapter/page numbers) produced a label long enough to wrap inside the 30 px footer container. Two-part fix in `formatStatusLine` + flow-mode label construction: (a) progress-bar glyph count capped at 10 cells (was 20) since the heavy/light horizontal box-drawing glyphs render wider than one cell on the G2 display; (b) flow-mode `infoText` dropped the redundant "Flow " prefix (RUN/PAUSE already conveys mode) and the "W N/M" word-index suffix (progress bar already shows within-page position). Worst-case label is now 50 cells — 9 cells of horizontal margin versus the 59-cell width.
