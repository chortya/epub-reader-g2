...
### Fixed
- **Status bar wraps to a second line**: After the v1.4.0 clock was prepended to the footer label, some pages (especially in flow mode and late-book paged pages with larger chapter/page numbers) produced a label long enough to wrap inside the 30 px footer container — observed as "a small scroll indicator for the lower frame" and "instead of the progress bar along the bottom you get a tiny scroll bar on the bottom right corner" (beta-tester reports against v1.3.1 and early v1.4.1 builds). The cause was purely horizontal: the label rendered wider than 576 px because the heavy/light horizontal box-drawing glyphs (`━`, `─`) render slightly wider than one monospace cell on the G2, and the clock prefix ate the remaining slack. Two-part fix in `formatStatusLine` + flow-mode label construction: (a) progress-bar glyph count capped at 10 cells (was 20); (b) flow-mode `infoText` dropped the redundant "Flow " prefix (RUN/PAUSE already conveys mode) and the "W N/M" word-index suffix (progress bar already shows within-page position). Worst-case label is now 50 cells — 9 cells of horizontal margin versus the 59-cell width, safe even if the box glyphs take up ~1.5 cell-widths.
- **Over-conservative blank-line reserve at cropped heights** (bug introduced mid-release): an earlier v1.4.1 attempt reduced blank padding by one line at cropped `textHeightPercent` values, which left a visible ~28 px empty gap between the reading text and the status bar. This was reverted once the true cause of the original symptom was pinpointed as the horizontal wrap above. Blank padding at cropped heights is now back to filling the full container, matching v1.3.1 visual behavior for reading text. The marginal 0.03 px vertical overflow at the cropped-height boundary is tolerated by the SDK in practice and never produced a visible symptom on its own.
